...
struct Fase{
	int Mapa[TamanhoX][TamanhoY];
};


void *BarcoMeio[2];			// |
void *IFundo;				// |
void *PonteDir[2];			// | - 0 para Imagem, 1 Para Mascara
void *Barril[2];			// |
void *Bau[2];				// |
void *SprPol[2][4];			// | - Mascara / Sprite
void *SprFan[2][4];			// |
void *Vitoria[2];			// |
void *Derrota[2];			// |
void *Botao[2][6];			// |
void *ImgMenu;				// |
void *Jog[2][2][22];		// | - No. Personagem/ Mascara/ Sprite
void *Logo[2];				// |

int Fim = 0;			//Condição de Fim de Jogo
int Atual = 0;			//Local que se está no jogo
int pg;					//contador do Anti-flicker
int tempo = 1;			//tempo usado para spawn de inimigos
int pl = 0;				//usado no spawn de polvos
int Dl = 35; 			//Delay
int L = 0;				//Menu = 0, Jogo = 1 ou Sair
int cMortos = 0;		//Contador de inimigos mortos
int Perdeu = 0;			//Boleano de Bau roubado
int Musica[4];			//usado como farol se ha musica tocando
int Bt = 0; 			//Botão usado nos Menus


Fase Onda[NFases];		// Fases
Person Jogador[2];		// Player 1 e 2
Cofre Tesouro[4];		// Tesouros

Inimigo Polvo[Plv];		// | - Inimigo 
...
void FTesouro(int X, int Y);
	//Controla os Baus no Mapa
	
void Personagem(int psX, int pxY);
...
int main(void){
	...
	initwindow(Larg,Altu,"Sea Cats",30,10);
...
void DescarregarImagens() {
	...
	for(M = 0; M <2; M++) {
		...
		for(n = 0;n < 22; n++) {
			free(Jog[0][M][n]);
			free(Jog[1][M][n]);
			if(n < 2) free(SprPol[M][n]);
			if(n < 6) free(Botao[M][n]);
		}
	}
...
void Inicio() {
	if(L == JInicio) {
		if(!Musica[0]){
			mciSendString("open .\\Audio\\Menu_Theme.mp3 type MPEGVideo alias Menu", NULL, 0, 0); 
		...
		putimage(Larg - 717, 0, ImgMenu, COPY_PUT);
		putimage(100, 30, Botao[1][0], OR_PUT);
		putimage(100, 30, Botao[0][0], AND_PUT);
		putimage(100, 30+70, Botao[1][4], OR_PUT);
		putimage(100, 30+70, Botao[0][4], AND_PUT);
	
	
		putimage(100, Altu - 150, Botao[1][3], OR_PUT);
		putimage(100, Altu - 150, Botao[0][3], AND_PUT);
		
		putimage(650, 110, Logo[1], OR_PUT);
		putimage(650, 110, Logo[0], AND_PUT);
		
		settextstyle(2, 0, 13);
		setbkcolor(RGB(0,0,0));
		outtextxy(760, 370, "Marcelo Hiroaki Hisatomi (Som)");
		outtextxy(760, 400, "Rodrigo Pereira Caetano da Silva (Prog.)");
		outtextxy(760, 430, "Virginia Cardoso de Toledo (Arte)");
		
		setbkcolor(RGB(0,50,100));
		
		if (Bt == 0) {
			putimage(50, 50, Jog[0][1][9], OR_PUT);
			putimage(50, 50, Jog[0][0][9], AND_PUT);
		
		}
		else if (Bt == 1) {
			putimage(50, 120, Jog[0][1][9], OR_PUT);
			putimage(50, 120, Jog[0][0][9], AND_PUT);
		}
		else if (Bt == 2) {
			putimage(50, Altu - 120, Jog[0][1][9], OR_PUT);
			putimage(50, Altu - 120, Jog[0][0][9], AND_PUT);
		}
		
		if(GetKeyState(VK_UP)&0x80 and Bt > 0) {
			sndPlaySound(".\\Audio\\Menu_Troca.wav", SND_ASYNC);
			Bt--;
			delay(200);
		}
		
		if(GetKeyState(VK_DOWN)&0x80 and Bt < 2 ) {
			sndPlaySound(".\\Audio\\Menu_Troca.wav", SND_ASYNC);
			Bt++;
			delay(200);
		}
...
void Pause() {
	if (L == JPause) {
		...
		if(GetKeyState(VK_ESCAPE)&0x80) {
			L = JInicio;
			mciSendString("close Fase1", NULL, 0, 0);
